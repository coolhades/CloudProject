...
public  class RootRequest {
    ...
    **/
    public RootRequest() {
        if (mBaseUrl.isEmpty()){
            Log.d(DebugTAG.TAG_ERROR, "mBaseUrl is Null");
            return;
        }
        ...
        public void onResponse(Call<T> call, Response<T> response) {
            ...
            if(response.isSuccessful()) {
                if(((BaseRetData)response.body()).status ==1) {
                    Log.d("TAG-Success", gson.toJson((T) ((BaseRetData) response.body()).data));
                    mCallback.onSuccess( (T) ((BaseRetData) response.body()).data );
                } else {
                    Log.d("TAG-Error", "出错:"+((BaseRetData)response.body()).message);
                    mCallback.onError(((BaseRetData)response.body()).message);
                }
            } else {
                mCallback.onFailure();
            }
