...
public abstract class BizRootOper {
    ...
    protected  void dealRefresh(BizActionItem item,  final Map<String, String> map){
        final String adress = ConstantSet.homeAddress + action.getBaseUrl() + item.getOpAction();
        ...
        VolleyRequestManager.getInstance().getRq().add(sq);
    }
...
//    }
    ...
    protected void dealSort(final BizActionItem item,  final Map<String, String> map){
        final String adress = ConstantSet.homeAddress + action.getBaseUrl() + item.getOpAction()+"?order="+item.getOpFlag()+item.getOpSort();
