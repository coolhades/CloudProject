package com.hades.mylibrary.base.ui.base.adapter;
...
import com.hades.mylibrary.base.ui.base.viewholder.EmptyHolder;
import com.hades.mylibrary.base.ui.base.viewholder.LoadView;
...
public abstract class RootAdapter extends RecyclerView.Adapter<RootViewHolder> {
    ...
    public List<BaseBean> rootDataList;
    //传进来几个ViewHolder 全部加入list
    public List<String> holdertypelist;//已去掉重复项
    Context mContext;

    public RootAdapter(List<BaseBean> data, Context context) {
    ...
    private void initViewType() {
        for (int i = 0; i < rootDataList.size(); i++) {
            holdertypelist.add(rootDataList.get(i).getBlockType());
        }
//      去掉重复的showCode 剩下的就是item的type
    ...
    public int getItemViewType(int position) {
        ...
        String type = rootDataList.get(position).getBlockType();
...
//        获取注册时关联的
    ...
    public void onBindViewHolder(RootViewHolder holder, int position) {
        //Viewholder 绑定单例
        LoadView.getInstance().loadView(holder, rootDataList.get(position), mContext);
    }
